...
class MGFNLoss(nn.Module):
    ...
    def forward(
        ...
        seperate = len(n_feat_magnitudes) / 2

        loss_cls = self.criterion(scores, labels)
        loss_con = self.contrastive(
            torch.norm(a_feat_magnitudes, p=1, dim=2),
            torch.norm(n_feat_magnitudes, p=1, dim=2),
            1,
        )  # try tp separate normal and abnormal
        loss_con_n = self.contrastive(
            torch.norm(n_feat_magnitudes[int(seperate) :], p=1, dim=2),
            torch.norm(n_feat_magnitudes[: int(seperate)], p=1, dim=2),
            0,
        )  # try to cluster the same class
        loss_con_a = self.contrastive(
            torch.norm(a_feat_magnitudes[int(seperate) :], p=1, dim=2),
            torch.norm(a_feat_magnitudes[: int(seperate)], p=1, dim=2),
            0,
        )
